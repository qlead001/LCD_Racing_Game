...
OBJD=$(filter-out $(PATHO)main.o, $(OBJS))
...
program-anim: $(PATHB)lcdAnimationTest.hex
	$(PROGRAM) -c $(PROGRAMMER) -p $(MMCU) $(VERBOSITY) -U $(MEMORY):w:$<

program-char: $(PATHB)lcdCharTest.hex
	$(PROGRAM) -c $(PROGRAMMER) -p $(MMCU) $(VERBOSITY) -U $(MEMORY):w:$<
...
$(PATHDO)%.elf: $(OBJD) $(PATHDO)%.o
	@$(AVR) $(DEBUGFLAGS) $(SIMFLAGS) $(FLAGS) $(INCLUDES) -o $@ $^

$(PATHB)%.hex: $(PATHDO)%.elf
	@$(OBJCOPY) $(OBJFLAGS) $< $@
...
clean:
	$(CLEAN) $(PATHO)*.o $(PATHDO)*.o $(PATHO)*.elf $(PATHDO)*.elf $(PATHB)*.hex
	$(CLEAN) $(PATHR)*.vcd
